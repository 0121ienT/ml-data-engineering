export const pipelineStages = [
  {
    ...
    phase: "Code check",
    requirement: [
      "Python: chọn 1 trong Black hoặc Ruff để format.",
      "JS/TS: chọn 1 trong Prettier hoặc ESLint.",
      "CI chạy lệnh check (vd 'black --check .' hoặc 'prettier --check .').",
    ],
  ...
  },
  {
    ...
    phase: "Code check",
    requirement: [
      "Chạy pytest (Python) hoặc jest/vitest (JS/TS) cho module chính.",
      "Có tối thiểu 5 test cho phần xử lý quan trọng nhất.",
      "Pipeline fail ngay nếu có 1 test fail.",
    ],
  ...
  },
  {
    ...
    requirement: [
      ...
      "Đặt tên image rõ ràng theo dạng <tên-app>:<tag> (vd 'emotion-api:latest').",
  ...
  },
  {
    ...
    phase: "Packaging",
    requirement: [
      "Chạy 'trivy image <tên-image>' và in output ra log CI.",
  ...
  },
  {
    ...
    phase: "Packaging",
    requirement: [
      "Push image lên Docker Hub hoặc GitHub Container Registry.",
      "Dùng tag 'latest' là đủ; ai muốn thêm tag git-sha thì làm bonus.",
      "Login bằng secret lấy từ CI, không hard-code username/password.",
    ],
  ...
  },
  {
    ...
    phase: "Deployment",
    requirement: [
      "Dùng docker compose để chạy app cùng DB/cache giả lập.",
      "Gọi 1-2 endpoint chính bằng curl, assert trả 200.",
      "Chạy 'docker compose down' để dọn container sau khi xong.",
    ],
    failFast: "endpoint không trả 200",
  },
  {
  ...
  },
  {
    ...
    phase: "Deployment",
    requirement: [
      "Pipeline dừng chờ 1 người trong nhóm bấm approve.",
      "Dùng GitHub Environment (Required reviewers) hoặc Jenkins input.",
    ],
  ...
  },
  {
    ...
    requirement: [
      ...
      "Tự test 1 endpoint sau khi up xong để chắc app còn sống.",
    ],
    failFast: "endpoint prod không trả 200 sau deploy",
  },
  {
    stage: "Notify",
    phase: "Notify",
    requirement: [
      ...
      "Message ghi: tên môi trường, commit-sha, link tới run của CI.",
    ],
    failFast: "—",
  },
];
